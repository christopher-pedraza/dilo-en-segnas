...
class ARExperience: ObservableObject {
    ...
    init() {
        ...
        lvl_buttonContainer.name = "lvlContainer_1"
        ...
        var panelContainer: Entity = Entity()
        panelContainer.name = "panelContainer"
        panelContainer.setScale(SIMD3<Float>(0.1, 0.1, 0.1), relativeTo: specificSceneAnchor)
        panelContainer.generateCollisionShapes(recursive: true)
        panelContainer.position = SIMD3(0, 0.4, 0)
        let rotation = simd_quatf(angle: .pi*3/2, axis: [0, 1, 0])
        panelContainer.transform.rotation = rotation
        panelContainer.setScale(SIMD3<Float>(0, 0, 0), relativeTo: specificSceneAnchor)
        ...
        /*
         El contenedor del panel tiene una entidad hija con el nombre "panel" porque los botones de
         las actividades deben estar pegados a la entidad del panel, en cambio el contenedor del nivel
    ...
    @objc func onTap(_ sender: UITapGestureRecognizer) {
        ...
        if let tappedEntity = arView.entity(at: tapLocation){
            ...
            // se toco el boton de un nivel, se accede al prefijo del nombre del asset porque se pudo haber tocado
            // el anillo o la cupula y ambos tienen al mismo padre en comun (nombre de la entidad)
            else if tappedEntity.parent!.name.hasPrefix("lvl_btn") {
                // isPanelVisible, lastPanelSeen
                if self.isPanelVisible == false { // mostrar panel del nivel
                    
                    // buscar el panel y mostrarlo
                    if let tempPanel = parentEntity!.findEntity(named: "panelContainer") {
    ...
    func updateLevel(actName: String) -> Void {
        ...
        if actName.hasPrefix("TH") {
            oldContainerName = "THContainer_" + actName.suffix(1)
            newButton = "TH_btn_true"
        } else if actName.hasPrefix("quiz") {
            oldContainerName = "quizContainer_" + actName.suffix(1)
            newButton = "quiz_btn_true"
        } else if actName.hasPrefix("video") {
            oldContainerName = "videoContainer_" + actName.suffix(1)
        ...
        if case let buttonContainer? = self.specificSceneAnchor.findEntity(named: "TH_btn_true") {
            ...
            print("TH encontrado")
            print("TH encontrado")
            //let position = buttonContainer.transform.matrix.columns.3
            //print("X: \(position.x), Y: \(position.y), Z: \(position.z)")
            let transform = buttonContainer.transform.matrix.columns
            print("X: \(transform.3.x), Y: \(transform.3.y), Z: \(transform.3.z)")
            print("scale X: \(length(transform.0)), scale Y: \(length(transform.1)), scale Z: \(length(transform.2))")
        } else {
        ...
        if case let buttonContainer? = self.specificSceneAnchor.findEntity(named: "video_btn_true") {
        ...
        if case let buttonContainer? = self.specificSceneAnchor.findEntity(named: "quiz_btn_true") {
            print("quiz encontrado")
            print("quiz encontrado")
            print("quiz encontrado")
            print("quiz encontrado")
            print("quiz encontrado")
            //let position = buttonContainer.transform.matrix.columns
            //print("X: \(position.3.x), Y: \(position.3.y), Z: \(position.3.z)")
            
            let transform = buttonContainer.transform.matrix.columns
            print("X: \(transform.3.x), Y: \(transform.3.y), Z: \(transform.3.z)")
            print("scale X: \(length(transform.0)), scale Y: \(length(transform.1)), scale Z: \(length(transform.2))")
        } else {
            print("quiz no encontrado")
            print("quiz no encontrado")
            print("quiz no encontrado")
            print("quiz no encontrado")
            print("quiz no encontrado")
        ...
        if let newObject = try? Entity.load(named: newButton) {
            ...
            if case let buttonContainer? = self.specificSceneAnchor.findEntity(named: oldContainerName) {
                ...
                if (completedActivities+1) == 3 {
                    ...
                    if let newPanel = try? Entity.load(named: "panel_true") {
                        if case var panelContainer = self.specificSceneAnchor.findEntity(named: "panelContainer") {
                            
                            newPanel.generateCollisionShapes(recursive: true)
                            
                            // obtener entidades hijas del panel hijo
                            var childPanelChildren: [Entity] = []
                            if let childPanel = panelContainer!.findEntity(named: "panel") {
                                for child in childPanel.children {
                                    childPanelChildren.append(child)
                                }
                            }
                            
                            // eliminar el objeto de blender (panel gris) de la lista de hijos del panel hijo
                            childPanelChildren.removeAll { (entity) -> Bool in
                                return entity.name == "panel_false"
                            }
                            
                            // eliminar todos los hijos del contenedor del panel
                            panelContainer!.children.removeAll()
                            
                            // agregar al nuevo panel los botones hijos completados del panel anterior conservando todas
                            // las propiedades de los hijos establecidas previamente
                            for child in childPanelChildren {
                                newPanel.addChild(child)
                            }
                            
                            // agregar el nuevo panel con todas las propiedades actualizadas al contenedor
                            panelContainer!.addChild(newPanel)
                             
                        }
                    }
                    
                    if let newLvl = try? Entity.load(named: "lvl_btn_true") {
                        if case let levelContainer = self.specificSceneAnchor.findEntity(named: "lvlContainer_1") {
                            
                           
                            newLvl.generateCollisionShapes(recursive: true)
                            
                            // eliminar todas las entidades del contenedor del nivel
                            levelContainer!.children.removeAll()
                            
                            // agregar el nuevo nivel actualizado
                            levelContainer!.addChild(newLvl)
                        }
                    }
                } // if de nivel completado
                
            } // busqueda del boton de la actividad terminada
        } // carga el boton de la actividad terminada
    } // final de la funcion updateLevel
}
